...
if( array_key_exists( $harington_current_item_count-1, $harington_portfolio_items ) ) {
	...
	$harington_data_attributes = "";
	if( $harington_portfolio_grid_layout == "parallax-grid" ){
		
		$harington_data_start = "0.0";
		$harington_data_end = "0.0";
		if( ( ($harington_current_item_count % 6) == 2 ) || ( ($harington_current_item_count % 6) == 4 ) ){
			
			$harington_data_start = "-0.3";
			$harington_data_end = "0.3";
		}
		if( ( ($harington_current_item_count % 6) == 1 ) || ( ($harington_current_item_count % 6) == 5 ) ){
			
			$harington_data_start = "0.0";
			$harington_data_end = "0.3";
		}
		// last item has to have end parallax zero
		if( $harington_current_item_count == count( $harington_portfolio_items ) ) {
			
			$harington_data_end = "0.0";
		}
		
		$harington_data_attributes = ' data-startparallax="' . $harington_data_start . '" data-endparallax="' . $harington_data_end . '"';
	}
	if( $harington_portfolio_grid_layout == "ladder-grid" ){
		
		$harington_data_scrub = "0.0001";
		if( ( $harington_current_item_count != 1 ) && ( $harington_current_item_count != count( $harington_portfolio_items ) ) ){
			
			if( ($harington_current_item_count % 2) == 0 ) {
			
				$harington_data_scrub = 0.25;
			}
			else {
				
				$harington_data_scrub = 0.5;
			}
		}
		
		$harington_data_attributes = ' data-scrub="' . esc_attr( $harington_data_scrub ) . '"';
	}
		
	$item_url = get_the_permalink( $harington_portfolio_item->post_id );
	
?>
						<div class="item trigger-item <?php echo esc_attr( $harington_item_classes ); ?>" data-color="<?php echo esc_attr( $harington_background_navigation ); ?>" <?php echo wp_kses( $harington_data_attributes, 'harington_allowed_html' ); ?>>
